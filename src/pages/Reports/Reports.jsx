...
const REPORTS_CONFIG = [
  ...
  },
  {
    ...
    endpoint: "/reports/growtags/",
  },
  {
    key: "complaints",
    title: "Total Complaints Report",
    endpoint: "/reports/complaints/",
  },
  {
    key: "sales-summary",
...
const ReportTable = ({ data, loading }) => {
  ...
  const assignTypes = [
    "",
    ...new Set(data.data.map((item) => item["ASSIGN TYPE"]).filter(Boolean)),
  ];
  const statusOptions = [
    "",
    ...new Set(data.data.map((item) => item["STATUS"]).filter(Boolean)),
  ];
  ...
  const filtered = useMemo(() => {
    return displayData.filter((row) => {
      ...
      if (
        ...
        return false;
      if (statusFilter && row["STATUS"] && row["STATUS"] !== statusFilter)
        return false;
      if (
        assignTypeFilter &&
        row["ASSIGN TYPE"] &&
        row["ASSIGN TYPE"] !== assignTypeFilter
      )
        return false;
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-4 md:p-6 w-full">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Total Records: {formatNumber(data.data.length)}
          </p>
        </div>
            ...
            {Object.entries(reportTotals).map(([key, value]) => {
              ...
              return (
                <div key={key} className="bg-white p-3 rounded-lg shadow-sm">
                  ...
                  <div
                    className={`text-lg font-semibold ${isCountKey ? "text-green-600" : "text-blue-600"}`}
                  >
                    {value}
                  </div>
                </div>
      ...
      {/* Filters */}
      {!["Sales Summary Report", "Profit Share Distribution Report"].includes(
        data.title,
      ) && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6 p-3 md:p-4 bg-gray-50 rounded-lg">
          ...
          {["Total Complaints Report", "Total Growth Tags Report"].includes(
            data.title,
          ) &&
          ...
          {["Total Complaints Report", "Total Customers Report"].includes(
            data.title,
          ) &&
            assignTypes.length > 1 && (
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  Assign Type
                </label>
            ...
            {filtered.map((row, idx) => (
              <tr key={idx} className="hover:bg-gray-50 transition-colors">
                {displayColumns.map((col) => {
                  ...
                  const isCountColumn = ["INVOICE COUNT", "COUNT"].includes(
                    col,
                  );
                  const isIdColumn = [
                    "ID",
                      ...
                      {isAmountColumn ? (
                        ...
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ["Closed", "Active", "Resolved"].includes(row[col])
                              ? "bg-green-100 text-green-800"
                              : row[col] === "In Progress"
                                ? "bg-yellow-100 text-yellow-800"
                                : ["Open", "Inactive", "Pending"].includes(
                                      row[col],
                                    )
      ...
      <div className="mt-4 text-sm text-gray-500 flex justify-between items-center">
        <span>
          Showing {filtered.length} of {data.data.length} records
        </span>
...
const Sidebar = ({
  ...
  return (
    ...
    >
      <div
        className={`p-4 ${collapsed ? "lg:p-3" : "lg:p-5"} border-b flex items-center justify-between`}
      >
        {!collapsed ? (
      ...
      <div
        className={`flex-1 p-4 ${collapsed ? "lg:px-2" : ""} overflow-y-auto`}
      >
