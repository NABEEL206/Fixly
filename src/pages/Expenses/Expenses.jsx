          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 p-2 bg-gray-50 rounded-lg font-semibold">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <span
                  className={`mt-1 px-3 py-1 rounded-full text-xs font-semibold inline-block ${getStatusColor(expense.status)}`}
                >
                  {expense.status}
                </span>
              </div>
...
const ExpenseTracker = () => {
  ...
  const canView = canAccess(role, PERMISSIONS.expenses.view);
  const canCreate = canAccess(role, PERMISSIONS.expenses.create);
  const canEdit = canAccess(role, PERMISSIONS.expenses.edit);
  const canDelete = canAccess(role, PERMISSIONS.expenses.delete);
  ...
  const addExpense = async () => {
    ...
    try {
      ...
      toast.success(`Expense "${formData.title}" added successfully`, {
        id: toastId,
      });
      resetForm();
    } catch (error) {
  ...
  const updateExpense = async () => {
    ...
    try {
      ...
      toast.success(`Expense "${formData.title}" updated successfully`, {
        id: toastId,
      });
      resetForm();
    } catch (error) {
      ...
      if (error.response?.status === 401) {
        ...
        const errorMessages = Object.entries(errorData)
          ...
          .join("\n");
        toast.error(errorMessages || "Failed to update expense", {
          id: toastId,
        });
      } else {
        toast.error(
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  toast.success(
                    `Expense "${expenseToDelete?.title}" deleted successfully`,
                    { id: dt },
                  );
                } catch (error) {
            ...
            <button
              onClick={async () => {
                ...
                const dt = toast.loading(
                  `Deleting ${selectedExpenses.length} expenses...`,
                );
                ...
                try {
                  ...
                  if (deleted.length > 0) {
                    setExpenses((prev) =>
                      prev.filter((exp) => !deleted.includes(exp.id)),
                    );
                    setSelectedExpenses([]);
                  ...
                  if (deleted.length === selectedExpenses.length) {
                    toast.success(
                      `${deleted.length} expense(s) deleted successfully`,
                      { id: dt },
                    );
                  } else {
                    if (deleted.length > 0) {
                    ...
                    if (forbidden.length > 0) {
                      toast.error(
                        `${forbidden.length} expense(s) could not be deleted (no permission)`,
                      );
                    }
                    if (notFound.length > 0) {
                      toast.error(
                        `${notFound.length} expense(s) not found — already deleted?`,
                      );
                    }
                  }
                } catch (error) {
  ...
  const handleSubmit = () => {
    ...
    if (editingExpense) {
      if (!canEdit) {
        toast.error("No permission to edit expense");
        return;
      }
      updateExpense();
    } else {
      if (!canCreate) {
        toast.error("No permission to create expense");
        return;
      }
      addExpense();
    }
  ...
  const filteredExpenses = expenses.filter((expense) => {
    ...
    const matchesCreatedBy =
      filterCreatedBy === "All" || expense.created_by === filterCreatedBy;
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredExpenses.map((expense) => (
              ...
              >
                ...
                <td className="px-4 py-4 text-center">
                  <span
                    className={`px-3 py-1 inline-flex text-xs font-semibold rounded-full ${getStatusColor(expense.status)}`}
                  >
                    {expense.status}
                  </span>
                </td>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`px-2 py-1 inline-flex text-xs leading-4 font-semibold rounded-full ${getStatusColor(expense.status)}`}
                >
                  {expense.status}
                </span>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Bulk Actions Bar */}
        {canDelete && selectedExpenses.length > 0 && (
